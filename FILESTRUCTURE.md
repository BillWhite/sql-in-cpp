...
* C++ source files:
  * `include/sql-in-cpp/sql-in-cpp.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/sql-in-cpp.cpp` is the main file that implements this library.
  * `app/sql-in-cpp_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/sql-in-cpp_t.cpp` contains the unit tests for the library.
